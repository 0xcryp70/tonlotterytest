...
const Task: React.FC = () => {
    ...
    useEffect(() => {
        const fetchInitialData = async () => {
            try {
                ...
                const taskResponse = await axios.get(`https://api2.tonlottery.info/api/getTaskStatus?telegramId=${telegramId}`);
                const taskData = taskResponse.data.tasks || [];

                setTasks(taskData);

                const referralResponse = await axios.get(`https://api2.tonlottery.info/api/getReferralUsers?telegramId=${telegramId}`);
    ...
    const completeTask = async (taskName: string) => {
        try {
            ...
            await axios.get(`https://api2.tonlottery.info/api/completeTask?telegramId=${telegramId}&telegramUsername=${telegramUsername}&taskName=${taskName}`);
            setTasks(prevTasks =>
                prevTasks.map(task =>
    ...
    const claimFreeTickets = async (milestone: number) => {
        ...
        try {
            const telegramId = window.Telegram.WebApp.initDataUnsafe.user.id;
            const response = await axios.get(`https://api2.tonlottery.info/api/buyFreeTickets?telegramId=${telegramId}&milestone=${milestone}`);
    ...
    const saveWalletDataToServer = async (data: { address: string, telegramUsername: string, telegramId: number | null, newTickets: number[], allTickets: number[], lastTransaction: string }) => {
        try {
            ...
            const response = await axios.get(`https://api2.tonlottery.info/api/saveWalletData${queryParams}`);
...
Task.preload = async () => {
    try {
        const telegramId = window.Telegram.WebApp.initDataUnsafe.user.id;
        await axios.get(`https://api2.tonlottery.info/api/getTaskStatus?telegramId=${telegramId}`);
        await axios.get(`https://api2.tonlottery.info/api/getReferralUsers?telegramId=${telegramId}`);
