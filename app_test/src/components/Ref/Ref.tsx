...
Your referral link: ${link}`;
  ...
  const fetchReferralUsers = async () => {
    try {
      ...
      const telegramId = initDataUnsafe?.user?.id;
      if (telegramId) {
        const response = await axios.get(`https://api2.tonlottery.info/api/getReferralUsers`, { params: { telegramId } });
...
Ref.preload = async () => {
  try {
    ...
    const telegramId = initDataUnsafe?.user?.id;
    if (telegramId) {
      await axios.get(`https://api2.tonlottery.info/api/getReferralUsers`, { params: { telegramId } });
