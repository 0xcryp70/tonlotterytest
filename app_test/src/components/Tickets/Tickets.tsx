...
const Tickets: React.FC = () => {
  ...
  const fetchActiveReferrals = async () => {
    try {
      const { data } = await axios.get(`https://api2.tonlottery.info/api/getReferralUsers?telegramId=${UserId}`);
  ...
  const updateBalanceWithReferrals = async () => {
    try {
        const response = await axios.get('https://api2.tonlottery.info/api/claimPrizeRef', {
            params: {
                telegramId: UserId
            }
        });
...
//        console.error('Error updating balance with referrals:', error);
  ...
  const handleClaimPrize = async () => {
    if (canClaimPrize) {
      try {
        const response = await axios.get('https://api2.tonlottery.info/api/claimPrize', {
          params: {
            telegramId: UserId,
            prizeAmount: 1
          }
...
Tickets.preload = async () => {
  try {
    ...
    if (UserId) {
      ...
      const balanceResponse = await axios.get(`https://api2.tonlottery.info/api/getUserBalance?telegramId=${UserId}`);
      console.log('Preloaded user balance:', balanceResponse.data.balance);

      // Preload active referrals
      const referralsResponse = await axios.get(`https://api2.tonlottery.info/api/getReferralUsers?telegramId=${UserId}`);
