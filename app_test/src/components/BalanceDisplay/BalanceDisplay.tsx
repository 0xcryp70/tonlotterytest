...
export const BalanceDisplay: React.FC<BalanceDisplayProps> = ({ walletAddress, memberLevel }) => {
  ...
  useEffect(() => {
    ...
    if (!lastResetDate || new Date(lastResetDate) < startOfWeekUTC) {
      setRewardClaimed(false);
      setRewardClaimedStatus(false);
      localStorage.setItem('lastResetDate', startOfWeekUTC.toISOString());
    }
  ...
  const handleCoinTouchStart = (e: React.TouchEvent<HTMLDivElement>) => {
    if (clickLocked || !showDailyReward) return;
  ...
  const handleCoinClick = async (e: React.TouchEvent<HTMLDivElement>) => {
    ...
    if (!showDailyReward) {
      ...
      if (clickCount >= 9) {
        ...
        setTimeout(() => {
          toast.info('Question: What is the smallest unit of Bitcoin called? Click the Morse code and get 0.1 TON');
          setClickLocked(false);
        }, 1000);
        setClickCount(0);
